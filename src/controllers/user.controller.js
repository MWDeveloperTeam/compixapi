...
export const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  const checkOldPassword = foundUser.isPasswordCorrect(oldPassword);
  if (!checkOldPassword) {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, {}, "password changed successifully"));
});
...
export const getUser = asyncHandler(async (req, res) => {
  ...
  const foundUser = await User.find({ email }).select(
    "-password -refreshToken"
  );
  if (!foundUser) {
    throw new ApiError(400, "user not found");
  }
  res.status(200).json(new ApiResponse(200, { foundUser }, "success"));
});
